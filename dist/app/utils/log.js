...
class Log {
    ...
    static logObject(obj, title = null) {
        if (title != null) {
            Log.logYellow("-- Obj: " + title);
            Log.logYellow(Log.makeLine(title.length + 10));
        }
        for (let key in obj) {
            if (obj.hasOwnProperty(key)) {
                Log.logYellow(key, obj[key]);
            }
        }
    }
    /**
    ...
    static logDebugCaller(caller, logMessage, logData = "") {
        if (app_settings_1.Settings.DebugMode) {
    ...
    static globalDebug(success, logData = null, ...logMessages) {
        ...
        if (success) {
            Log.logGreen(message, logData);
        }
        else {
            Log.logRed(message, logData);
        }
    }
    /*****************
     */
    ...
    static logGreenPrefix(prefix, logMessage, logData = null) {
        let logStr = Log.prepStr(logMessage, logData);
